...
- **VFFシューズ** — VFFブランドのうちシューズのみ（ソックス・Furoshiki等を除く）の総足数・月次推移、販売チャネル5区分（オンラインストア／委託販売(オフライン)／直営実店舗／Central百貨店／Siam Discovery）のシェア、性別セグメント分布（サイズ表記W/Mからの推定値）、モデル別足数ランキング
